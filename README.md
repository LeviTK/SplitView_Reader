...
Arc 浏览器风格的页面内分屏阅读 Chrome 插件，支持智能批量框选网页内容，解析为富文本或 Markdown，提供纯净的沉浸式阅读体验。

## 🌟 核心特性

- **页面内分屏**：不依赖独立窗口或侧边栏，直接在当前页面右侧（40% 宽度）展开阅读面板，左侧保留源网页（60% 宽度）。
- **智能批量选择**：
  - **悬停预览**：自动识别并高亮当前元素，同时基于同级结构（Tag + `data-testid`/Class）智能识别相似元素候选。
  - **批量锁定**：单击锁定“批量模式”，默认选中所有候选元素。
  - **自由切换**：支持点击切换单个元素的选中状态。
  - **一键确认**：按 `Enter` 键确认提取，按 `Esc` 键取消或退出。
- **自动展开内容**：内置站点规则引擎，支持在提取前自动点击“显示更多”、“Read more”按钮（如 X/Twitter），获取完整内容。
- **双模式阅读**：
  - **富文本模式**：保留原始 HTML 结构与核心样式。
...
- **悬停 (Hover)**：
  ...
  - `identifyBatchCandidates` 基于 DOM 结构识别同级候选元素（虚线框覆盖）。
- **锁定 (Lock)**：
  - 第一次点击进入 `Batch Mode`，锁定候选集并默认全选（实线框高亮）。
  - 后续点击切换单个元素的选中状态。
- **确认 (Confirm)**：
  - 按 `Enter` 触发提取流程。
  - `expandContent` 根据规则自动展开内容（如模拟点击展开按钮）。
  - `extractElementContent` 净化 DOM（移除脚本/广告/相对路径转绝对路径）。
  - 拼接所有选中内容，调用 `showSplitView`。

### 3. 分屏阅读 (Split View Panel)
- **布局**：通过 CSS 调整 `html/body` 宽度至 60vw，右侧 40vw 插入 `#splitview-panel`。
- **渲染**：
  - 富文本：直接插入净化后的 HTML。
  - Markdown：使用 `markdown-it` 转换 HTML。
- **工具栏**：提供模式切换、复制、源码复制、PDF 导出与关闭功能。
...
| `content.js` | 核心逻辑 | 实现元素检查器、批量选择逻辑、站点规则加载、内容提取、分屏面板渲染与交互。 |
| `content.css` | 样式 | 定义高亮框、批量选择覆盖层、分屏面板及阅读内容的样式。 |
...
5. 打开任意网页（推荐 Twitter/X 帖子列表测试批量选择），点击插件图标即可使用。
